...
from rpc_manager import MultiRPCManager
...
RPC_PROVIDERS = [QUICKNODE_URL] + BACKUP_RPCS
...
GAS_EXECUTE_USD_MAX = 0.10
GAS_SKIP_USD        = 0.12
APESWAP_SCAN_PAIRS  = 50
...
TRIANGLE_PATHS = [
    ...
    # User-requested additional triangle paths
    ["BNB",  "USDT",  "XRP",   "BNB"],
    ["BNB",  "ETH",   "BTCB",  "BNB"],
    ["USDT", "SOL",   "BNB",   "USDT"],
    ["BNB",  "ADA",   "USDT",  "BNB"],
...
apeswap         = w3.eth.contract(address=Web3.to_checksum_address(APESWAP_ROUTER),  abi=ROUTER_ABI)
...
apeswap_factory = w3.eth.contract(address=Web3.to_checksum_address(APESWAP_FACTORY), abi=FACTORY_ABI)
...
async def _scan_dex(dex_name, pair_limit=None):
    opportunities = []
    router = ROUTERS.get(dex_name)
    if not router:
        return opportunities

    pairs = BOT_SCAN_PAIRS if pair_limit is None else BOT_SCAN_PAIRS[:pair_limit]
    for token_in_name, token_out_name in pairs:
        if token_in_name not in TOKENS or token_out_name not in TOKENS:
            continue
        ...
        price_here = get_price(router, token_in, token_out, amount_in)
        if price_here <= 0:
            continue

        for other_name, other_router in ROUTERS.items():
            if other_name == dex_name:
                continue
            other_price = get_price(other_router, token_in, token_out, amount_in)
            if other_price <= 0:
                continue

            buy_dex, sell_dex = (dex_name, other_name) if price_here < other_price else (other_name, dex_name)
            buy_price, sell_price = (price_here, other_price) if price_here < other_price else (other_price, price_here)
            gap_pct = ((sell_price - buy_price) / buy_price) * 100 if buy_price > 0 else 0.0
            gross_profit = float(w3.from_wei(max(sell_price - buy_price, 0), "ether"))
            gas_cost = 0.10
            flash_fee = gross_profit * 0.0009
            ...
            opportunities.append({
                ...
                "flash_fee": round(flash_fee, 6),
                "amount": float(MAX_TRADE_SIZE),
            })

    return opportunities


async def scan_pancakeswap():
    return await _scan_dex("PancakeSwap")


async def scan_biswap():
    return await _scan_dex("Biswap")


async def scan_apeswap():
    # Respect requested ApeSwap pair-scan cap
    limit = min(APESWAP_SCAN_PAIRS, len(BOT_SCAN_PAIRS))
    return await _scan_dex("ApeSwap", pair_limit=limit)


def should_execute_trade(gross_profit_usd, gas_cost_usd, flash_fee_usd, trade_amount_usd):
    if gas_cost_usd > GAS_SKIP_USD:
        return False, f"Gas too high (${gas_cost_usd:.4f} > ${GAS_SKIP_USD:.2f})"
    if gas_cost_usd >= GAS_EXECUTE_USD_MAX:
        return False, f"Gas not favorable (${gas_cost_usd:.4f} >= ${GAS_EXECUTE_USD_MAX:.2f})"

    slippage_cost = float(trade_amount_usd) * SLIPPAGE_TOLERANCE
    net = float(gross_profit_usd) - float(gas_cost_usd) - float(flash_fee_usd) - slippage_cost
    if net < MIN_PROFIT_USD:
        return False, f"Net ${net:.4f} below min ${MIN_PROFIT_USD:.2f}"
    return True, f"Net ${net:.4f} (gas ${gas_cost_usd:.4f}, slip ${slippage_cost:.4f})"


async def execute_flash_loan_trade(opp):
    buy_dex = opp["buy_dex"]
    sell_dex = opp["sell_dex"]
    token_in = TOKENS[opp["token_in"]]
    token_out = TOKENS[opp["token_out"]]
    gap_pct = float(opp.get("gap_pct", 0.0))

    if gap_pct >= 1.0:
        return execute_flash_loan(buy_dex, sell_dex, token_in, token_out, gap_pct)

    amount_in = w3.to_wei(MAX_TRADE_SIZE, "ether")
    buy_price = get_price(ROUTERS[buy_dex], token_in, token_out, amount_in)
    return execute_normal_trade(buy_dex, sell_dex, token_in, token_out, amount_in, buy_price)
...
async def main_bot_loop():
    ...
    while True:
        ...
        try:
            ...
            print("📡 Scanning PancakeSwap...")
            ps_opps = await scan_pancakeswap()
            opportunities.extend(ps_opps)

            print("📡 Scanning Biswap...")
            bs_opps = await scan_biswap()
            opportunities.extend(bs_opps)

            print("📡 Scanning ApeSwap...")
            ap_opps = await scan_apeswap()
            opportunities.extend(ap_opps)
            ...
            for opp in opportunities:
                approved, reason = should_execute_trade(
                    gross_profit_usd=opp["gross_profit"],
                    gas_cost_usd=opp["gas_cost"],
                    flash_fee_usd=opp["flash_fee"],
                    trade_amount_usd=opp["amount"],
                )

                if approved:
                    print(f"✅ Executing trade: {opp['pair']} | {reason}")
                    await execute_flash_loan_trade(opp)
                    executed += 1
                    break
                else:
...
if __name__ == "__main__":
    print("🚀 ArbiBot Pro Starting...")
    print("🌐 Connecting to RPC providers...")
    rpc = MultiRPCManager(RPC_PROVIDERS)
    print("✅ RPC connected")
